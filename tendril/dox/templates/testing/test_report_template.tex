...
\rowcolor[gray]{0.9} & \normalsize\textbf{%{{ suite.desc %}}} & \textbf{%{{ suite.title %}}} & {\footnotesize %{{ suite.ts %}}} & \textbf{%{{ suite.passed %}}} \tabularnewline\hline
  ...
  %{ for test in suite.tests %}
    ...
    &  &  \begin{tabular}{>{\centering\arraybackslash}p{0.25\linewidth}|>{\centering\arraybackslash}p{0.3\linewidth}|>{\centering\arraybackslash}p{0.3\linewidth}}
      %{ if not test.passfailonly %}
      %{ for l in test.lines %}
	\hline
         {\small %{{ l.desc %}}} & %{ if l.expected %} {\small %{{ l.expected %}}} & \textbf{\small %{{ l.measured %}}} %{ else %} \multicolumn{2}{r}{\small %{{ l.measured %}}} %{ endif %} \tabularnewline
      %{ endfor %}
      %{ endif %}
      \end{tabular}
      &  &  \tabularnewline
...
%{ for instrument in stage.instruments|dictsort(False, 'value') %}
  %{{ instrument.1 %}} & \multicolumn{4}{l|}{%{{ instrument.0 %}}} \tabularnewline
%{ endfor %}
\hline
\end{tabularx}
\end{center}
...
\begin{figure}[h!]
  %{ for graph in stage.graphs %}\begin{subfigure}[b]{0.33\textwidth}
