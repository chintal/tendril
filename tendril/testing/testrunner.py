...
def run_test_offline(serialno, devicetype):
    ...
    user_input = raw_input("Print to Paper [y/N] ?: ").strip()
    if user_input.lower() in ['y', 'yes', 'ok', 'pass']:
        publish_and_print(serialno, devicetype, suites=suites, print_to_paper=True)
    else:
        publish_and_print(serialno, devicetype, suites=suites, print_to_paper=False)
