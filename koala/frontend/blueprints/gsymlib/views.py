...
import koala.gedaif.gsymlib

from . import gsymlib as blueprint

from koala.utils.fs import get_path_breadcrumbs
from koala.utils.fs import Crumb
...
def get_geda_browser_context(path):
    ...
    symbols = koala.gedaif.gsymlib.gen_symlib(path,
...
def get_geda_symbol_context(ident):
    symbol = koala.gedaif.gsymlib.get_symbol(ident, get_all=True)
...
def get_geda_generator_context(gen):
    generator = koala.gedaif.gsymlib.get_generator(gen)
    navpath = os.path.relpath(generator.fpath, GEDA_SYMLIB_ROOT)
    navpath = os.path.splitext(navpath)[0] + '.gen'
    genobj = koala.gedaif.gsymlib.GSymGeneratorFile(generator.fpath)
...
def main(path=None, ident=None, gen=None):
    ...
    if ident is not None:
        ident = unquote(ident)
        if ident in koala.gedaif.gsymlib.gsymlib_idents:
    ...
    if gen is not None:
        gen += '.gen'
        gen = unquote(gen)
        if gen in koala.gedaif.gsymlib.generator_names:
