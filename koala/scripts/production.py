...
import koala.boms.outputbase

import koala.inventory.electronics
import koala.dox.production
import koala.dox.indent
import koala.dox.docstore
import koala.dox.labelmaker
import koala.gedaif.conffile


from koala.entityhub import projects
from koala.entityhub import serialnos

from koala.utils.pdf import merge_pdf
from koala.utils.progressbar.progressbar import ProgressBar
...
if __name__ == "__main__":
    ...
    for k, v in data['cards'].iteritems():
        bom = koala.boms.electronics.import_pcb(projects.cards[k])
    ...
    cobom = koala.boms.outputbase.CompositeOutputBom(bomlist)
    ...
    for pbidx, line in enumerate(cobom.lines):
        ...
        for idx, descriptor in enumerate(cobom.descriptors):
            ...
            avail = koala.inventory.electronics.get_total_availability(line.ident)
            if line.columns[idx] == 0:
                continue
            if avail > line.columns[idx]:
                koala.inventory.electronics.reserve_items(line.ident, line.columns[idx], earmark)
            elif avail > 0:
                koala.inventory.electronics.reserve_items(line.ident, avail, earmark)
    ...
    title = data['title']
    indentpath, indentsno = koala.dox.indent.gen_stock_idt_from_cobom(orderfolder,
                                                                      indentsno, title,
                                                                      data['cards'], cobom)
    if REGISTER is True:
        koala.dox.docstore.register_document(indentsno, indentpath, 'INVENTORY INDENT', efield=title)
    else:
    ...
    if REGISTER is True:
        ...
        koala.dox.docstore.register_document(indentsno, os.path.join(orderfolder, 'cobom.csv'),
    ...
    for card, qty in sorted(data['cards'].iteritems()):
        try:
            ...
            raise KeyError
        cardconf = koala.gedaif.conffile.ConfigsFile(cardfolder)
        ...
        for idx in range(qty):
            ...
            if genlabel is True:
                for label in labels:
                    koala.dox.labelmaker.manager.add_label(label['code'], label['ident'], sno)
            if genmanifest is True:
                ampath = koala.dox.production.gen_pcb_am(cardfolder, card,
                                                         manifestsfolder, sno,
                                                         productionorderno=PROD_ORD_SNO,
                                                         indentsno=indentsno)
                manifestfiles.append(ampath)
                if REGISTER is True:
                    koala.dox.docstore.register_document(sno, ampath, 'ASSEMBLY MANIFEST')

    production_order = koala.dox.production.gen_production_order(orderfolder, PROD_ORD_SNO,
                                                                 data, snos,
                                                                 sourcing_orders=SOURCING_ORDERS,
                                                                 root_orders=ROOT_ORDERS)
    labelpaths = koala.dox.labelmaker.manager.generate_pdfs(orderfolder, force=FORCE_LABELS)
    ...
    if REGISTER is True:
        if os.path.exists(os.path.join(orderfolder, 'device-labels.pdf')):
            koala.dox.docstore.register_document(PROD_ORD_SNO, os.path.join(orderfolder, 'device-labels.pdf'),
                                                 'DEVICE LABELS', data['title'])
        koala.dox.docstore.register_document(PROD_ORD_SNO, production_order, 'PRODUCTION ORDER', data['title'])
        koala.dox.docstore.register_document(PROD_ORD_SNO, orderfile, 'PRODUCTION ORDER YAML', data['title'])
    else:
    ...
    if REGISTER is True:
        koala.dox.docstore.register_document(PROD_ORD_SNO, os.path.join(orderfolder, 'snomap.yaml'),
                                             'SNO MAP', data['title'])
