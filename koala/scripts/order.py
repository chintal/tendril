...
from koala.gedaif import projfile

from koala.utils.progressbar.progressbar import ProgressBar
import koala.inventory.guidelines
...
base_tf = koala.inventory.electronics.inventory_locations[0]._reader.tf
...
if LOAD_PRESHORT is True:
    ...
    if os.path.exists(os.path.join(orderfolder, 'preshort.csv')):
        with open(os.path.join(orderfolder, 'preshort.csv'), 'r') as f:
            ...
            obom_descriptor = koala.boms.outputbase.OutputElnBomDescriptor('PRESHORT',
                                                                           os.path.join(INSTANCE_ROOT, 'scratch'),
                                                                           'PRESHORT', None)
            obom = koala.boms.outputbase.OutputBom(obom_descriptor)
            ...
            for line in reader:
                ...
                if base_tf.has_contextual_repr(line[1]):
                    device, value, footprint = koala.conventions.electronics.parse_ident(base_tf.get_canonical_repr(line[1]))
                    item = koala.boms.electronics.EntityElnComp()
        ...
        if os.path.exists(external_req_file):
            ...
            with open(external_req_file, 'r') as f:
                ...
                for head in header[1:]:
                    ...
                    obom_descriptor = koala.boms.outputbase.OutputElnBomDescriptor(head,
                                                                                   os.path.join(INSTANCE_ROOT, 'scratch'),
                                                                                   head, None)
                    obom = koala.boms.outputbase.OutputBom(obom_descriptor)
                    oboms.append(obom)
                ...
                for line in reader:
                    ...
                    device, value, footprint = koala.conventions.electronics.parse_ident(base_tf.get_canonical_repr(line[0]))
    ...
    if USE_STOCK is True:
        ...
        if PRIORITIZE is False:
            ...
            if line.subset_qty(immediate_idxs) > 0:
                ...
                shortage = 0
    if shortage > 0:
        result = koala.sourcing.electronics.order.add(line.ident, line.quantity, shortage, orderref)
        if result is False:
...
koala.sourcing.electronics.order.collapse()
koala.sourcing.electronics.order.rebalance()
koala.sourcing.electronics.order.generate_orders(orders_path)
koala.sourcing.electronics.order.dump_to_file(os.path.join(orderfolder, 'shortage.csv'), include_others=False)
koala.inventory.electronics.export_reservations(reservations_path)
