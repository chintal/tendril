...
class GedaSymbol(object):
    ...
    def _img_repr(self):
        ...
        if os.path.exists(self._img_repr_path):
            if koala.utils.fs.get_file_mtime(self._img_repr_path) > koala.utils.fs.get_file_mtime(self.fpath):
                return
    ...
    @property
    def ident(self):
        return koala.conventions.electronics.ident_transform(self.device,
                                                             self.value,
                                                             self.footprint)
    ...
    def sym_ok(self):
        rval = False
        if self.device in koala.conventions.electronics.DEVICE_CLASSES:
            rval = True
        return rval
    ...
    @property
    def is_wire(self):
        return koala.conventions.electronics.fpiswire(self.device)

    @property
    def is_modlen(self):
        return koala.conventions.electronics.fpismodlen(self.device)

    @property
    def datasheet_url(self):
        # TODO This can be cached
        try:
            from koala.sourcing.electronics import vendor_list
            from koala.inventory.guidelines import electronics_qty
            dkv = vendor_list[0]
    ...
    def idents(self):
        ...
        if not self.generator.values:
            return None
        return [koala.conventions.electronics.ident_transform(self.device, v, self.footprint)
...
def find_capacitor(capacitance, footprint, device='CAP CER SMD', voltage=None):
    for symbol in gsymlib:
        if symbol.device == device and symbol.footprint == footprint:
            cap, volt = koala.conventions.electronics.parse_capacitor(symbol.value)
            sym_capacitance = koala.conventions.electronics.parse_capacitance(cap)
...
def find_resistor(resistance, footprint, device='RES SMD', wattage=None):
    ...
    for symbol in gsymlib:
        if symbol.device == device and symbol.footprint == footprint:
            res, watt = koala.conventions.electronics.parse_resistor(symbol.value)
            sym_resistance = koala.conventions.electronics.parse_resistance(res)
...
def export_gsymlib_audit():
    ...
    outf = koala.utils.fs.VersionedOutputFile(auditfname)
