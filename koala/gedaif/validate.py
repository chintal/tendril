...
def check(projectfolder):
    ...
    try:
        gpf = koala.gedaif.projfile.GedaProjectFile(projectfolder)
    ...
    bom = koala.boms.electronics.import_pcb(projectfolder)
    ...
    for config in configs:
        ...
        for line in obom.lines:
            ident = line.ident
            if not koala.gedaif.gsymlib.is_recognized(ident) and not ident.startswith('PCB '):
                logger.warning("Component not recognized by gsymlib : " + line.ident + " " + str(line.refdeslist))
            elif not ident.startswith('PCB '):
                symbol = koala.gedaif.gsymlib.get_symbol(line.ident)
