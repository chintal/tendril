...
from koala.utils.types import currency
...
class VendorDigiKey(vendors.VendorBase):
    ...
    def search_vpnos(self, ident):
        device, value, footprint = koala.conventions.electronics.parse_ident(ident)
        ...
        try:
            if device.startswith('RES') or device.startswith('POT') or \
                ...
                if koala.conventions.electronics.check_for_std_val(ident) is False:
    ...
    def _get_pas_vpnos(self, device, value, footprint):
        ...
        if devtype == 'resistor':
            ...
            lvalues = koala.conventions.electronics.parse_resistor(value)
        elif devtype == 'capacitor':
            loptions = (('capacitance', "Capacitance", 'equals'),
                        ('voltage', "Voltage - Rated", 'equals'))
            lvalues = koala.conventions.electronics.parse_capacitor(value)
        elif devtype == 'crystal':
            loptions = (('frequency', "Frequency", 'equals'),)
            lvalues = [koala.conventions.electronics.parse_crystal(value)]
        else:
            raise ValueError
